...
# Кнопка
 в більшості кнопок ```div.default-button ``` всередині є ```<div class='loading-indicator-div' style='display:none;'> ```. Якщо потрыбно показати процес завантаження після кліку - змінити на display:block;
# Попапи
